...
case class PersonRepository()(implicit executionContext: ExecutionContext)
    extends Repository[DBIO, Int, PersonRow] {

  private lazy val persons = PersonTable.table

  def save(person: PersonRow): DBIO[PersonRow] =
    saveAndGetId(person).map { id => person.focus(_.id).replace(id.some) }
  ...
  private def filterById(id: Int): Query[PersonTable, PersonRow, Seq] =
